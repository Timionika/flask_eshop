...
from eshop.view.order_schemas import OrderCreateDtoSchema, OrderSchema, OrderGetManyParams, ProductCreateDtoSchema, ProductSchema, ProductGetManyParams
...
def product_create_endpoint():
    try:
        prodect_create_dto = ProductCreateDtoSchema().load(request.json)
    except ValidationError as err:
        return err.messages, 400

    try:
        product = product_create(
            product_name=prodect_create_dto['product_name'],
            product_price=prodect_create_dto['product_price']
        )
    except Exception as e:
        return {
            "error": str(e)
